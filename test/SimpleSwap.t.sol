...
contract SimpleSwap is BaseDeploy {
    ...
    function setUp() public override {
        ...
        IERC20(ORETH).approve(address(swapRouter), type(uint256).max);
        (uint256 amountA1, uint256 amountB1, uint256 liquidity1) = addLiquidity(USDB, ORETH, 10000, 100000);
        (uint256 amountA2, uint256 amountB2, uint256 liquidity2) = addLiquidity(tokens[0], ORETH, 10000, 1000);

        vm.stopBroadcast();
    }

    function test_SwapUSDBtoETHtoORETH() public {
        vm.startBroadcast(deployer);
        ...
        console2.log("Swap USDB to ORETH:", amounts[0], "-", amounts[2]);

        console2.log("----------------------- start test USDB to ETH ------------------");
        amounts = swapToken(deployer, USDB, ORETH, amountUSDB / 10, 0, block.timestamp);
        console2.log("Swap USDB to ETH:", amounts[0], "-", amounts[1]);

        console2.log("----------------------- start test ETH to token0 ------------------");
        amounts = swapToken(deployer, ORETH, USDB, amountUSDB / 10, 0, block.timestamp);
        console2.log("Swap USDB to ETH:", amounts[0], "-", amounts[1]);
